<template>
    <section class="h-[90dvh] p-10">
        <!-- profile -->
        <div v-if="user" class="flex items-center gap-x-5 font-poppins md:w-3/6 lg:w-fit md:mx-auto">
            ...
            <div>
                <h1 class="text-black text-xl md:text-2xl font-semibold uppercase">{{ user.name }}</h1>
        ...
        <!-- receipt -->
        <div v-if="applicant" class="flex items-center justify-between md:w-2/6 xl:w-2/6 md:mx-auto border border-custom-primary shadow-inner shadow-red-200 rounded-full px-4 py-1 mt-20">
        ...
        <div v-else class="flex items-center gap-x-5 justify-between md:w-3/6 lg:w-2/6 md:mx-auto px-6 py-2 mt-20">
