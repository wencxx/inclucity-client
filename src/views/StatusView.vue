    ...
    <div class="container md:w-2/3 lg:w-2/5 p-10 font-poppins flex flex-col items-center gap-y-14 mx-auto">
        ...
        <div v-if="applicant && applicant.status !== 'expired'" class="flex items-center justify-between w-full">
            <div class="flex flex-col items-center">
                <Icon icon="clarity:form-line"  class="text-5xl text-green-900" :class="{ '!text-orange-600': applicant }"/>
                <p class="text-sm">Filled out form</p>
            </div>
            <Icon icon="ph:arrow-right"  class="text-5xl text-blue-700"/>
            <div class="flex flex-col items-center">
                <Icon icon="uim:process"  class="text-5xl text-green-900" :class="{ '!text-orange-600': applicant && applicant.status !== 'pending'}"/>
                <p class="text-sm">Processed</p>
            </div>
            <Icon icon="ph:arrow-right"  class="text-5xl text-blue-700"/>
            <div class="flex flex-col items-center">
                <Icon icon="bitcoin-icons:verify-filled" class="text-6xl text-green-900" :class="{'!text-orange-600': applicant && applicant?.status === 'approved'}" />
        ...
        <div v-if="applicant && applicant.status !== 'expired'" class="border w-full flex flex-col gap-y-3 p-5 rounded-xl shadow">
            ...
            <div class="flex justify-between items-center">
                ...
                <p v-if="applicant && applicant.reasonForRejection != 'Invalid Forms' && applicant.status !== 'pending'" class="text-orange-600">Approved</p>
                <p v-else-if="applicant &&  applicant.status === 'pending'" class="text-orange-600">Processing</p>
                <p v-else class="text-red-500">Rejected</p>
            </div>
            <div class="flex justify-between items-center">
                <h1 class="font-medium text-xl">1x1 Picture</h1>
                <p v-if="applicant && applicant.reasonForRejection != 'Invalid 1x1 Picture' && applicant.status !== 'pending'" class="text-orange-600">Approved</p>
                <p v-else-if="applicant &&  applicant.status === 'pending'" class="text-orange-600">Processing</p>
                <p v-else class="text-red-700">Rejected</p>
            </div>
            <div class="flex justify-between items-center">
                <h1 class="font-medium text-xl">Medical Certificate</h1>
                <p v-if="applicant && applicant.reasonForRejection != 'Invalid Medical Certificate' && applicant.status !== 'pending'" class="text-orange-600">Approved</p>
                <p v-else-if="applicant &&  applicant.status === 'pending'" class="text-orange-600">Processing</p>
                <p v-else class="text-red-700">Rejected</p>
            </div>
            <div class="flex justify-between items-center">
                <h1 class="font-medium text-xl">Barangay Certificate</h1>
                <p v-if="applicant && applicant.reasonForRejection != 'Invalid Barangay Certificate' && applicant.status !== 'pending'" class="text-orange-600">Approved</p>
                <p v-else-if="applicant &&  applicant.status === 'pending'" class="text-orange-600">Processing</p>
                <p v-else class="text-red-700">Rejected</p>
            </div>
